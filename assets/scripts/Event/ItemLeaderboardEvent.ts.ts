...
import { ClansData, LeaderboardItemDTO } from '../Interface/DataMapAPI';
import { AvatarIconHelper } from '../Clan/AvatarIconHelper';
const { ccclass, property } = _decorator;

@ccclass('ItemLeaderboardEvent')
export class ItemLeaderboardEvent extends Component {
    ...
    setData(data: LeaderboardItemDTO, isClanMode: boolean) {
        this.nameLabel.string = isClanMode ? `${data.clan_name}` : `${data.user_display_name}`;
        const score = data.score;
        this.totalScoreLabel.string = score > 0 ? score.toString() : '--';
        this.setRankIcon(data.rank ? data : null);
        // const avatar = data.avatar_url?.trim() || "avatar_1";
        // this.avatarSprite.setAvatar(avatar);
    }
    
    setRankIcon(data: LeaderboardItemDTO) {
