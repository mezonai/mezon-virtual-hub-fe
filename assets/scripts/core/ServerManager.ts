    ...
    public async joinRoom(roomName = "my_room") {
        ...
        this.room.onMessage("onExchangeDiamondToCoin", (data) => {
            ...
            if (UserMeManager.Get) {
                const msg = `<color=#FF0000>${Utilities.convertBigNumberToStr(Utilities.convertBigNumberToStr(Math.abs(diamondChange)))} Coin</color> đã được chuyển thành <color=#00FF00>${coinChange} Diamond</color>`;
